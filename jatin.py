...
app.config['MYSQL_PASSWORD']='23110147'
...
def a_2():
    if request.method == "POST":
        cursor = mysql.connection.cursor()
        cursor.execute("select * from d11 where bond_number = %s",(request.form['box'],))
...
def e_1():
     # Initialize data_final as an empty list
    
    if request.method == "POST":
        cursor = mysql.connection.cursor()
        if request.form['filter_type'] == 'Name of the Purchaser':
            cursor.execute("SELECT * FROM d11 WHERE Name_of_the_Purchaser = %s", (request.form['filter_value'],))
            data = cursor.fetchall()
            data_final = [] 
            for row in data:
                if str(row[7]) == request.form['filter_value']:  # Assuming column 7 is the Purchaser name
